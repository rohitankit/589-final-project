...
class ModelEvaluation:
    ...
    def addInstanceEvaluation(self, trueClass, predictedClass):
        ...
        self.evaluationMatrix[int(trueClass) - int(self.labelOffset)][
            predictedClass - self.labelOffset
        ] += 1
        ...
        for predictedClassIdx in range(self.evaluationDimensions):
            ...
            for trueClassIdx in range(self.evaluationDimensions):
                if predictedClassIdx != trueClassIdx:
                    falsePositives += self.evaluationMatrix[trueClassIdx][
                        predictedClassIdx
                    ]
    ...
    def getLabelRecalls(self):
        ...
        for trueClassIdx in range(self.evaluationDimensions):
            ...
            for predictedClassIdx in range(self.evaluationDimensions):
                if predictedClassIdx != trueClassIdx:
                    falseNegatives += self.evaluationMatrix[trueClassIdx][
                        predictedClassIdx
                    ]
    ...
    def getF1Score(self):
        ...
        for i in range(len(labelPrecisions)):
            F1Total += ModelEvaluation.getHarmonicAverage(
                labelRecalls[i], labelPrecisions[i]
            )
